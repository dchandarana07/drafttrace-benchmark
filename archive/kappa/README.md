> **Archived.** This study was released with an earlier version of this
> repository, which held the provenance benchmark for the DraftTrace paper. It
> is kept here unchanged because the paper cites it; everything else from that
> release is in this repository's git history. Paths mentioned below refer to
> the layout of that release, not to the current tree — `score.py` and its
> inputs are all present in this folder and still run.
>
> Reproduce: `python3 archive/kappa/score.py archive/kappa/adjudications.json`
